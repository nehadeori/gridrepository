...
namespace JQxGrid.Controllers
{
    public class OrdersController : Controller
    {
        ...
        public JsonResult GetValues(string sidx, string sord, int page, int rows) //Gets the todo Lists.  
        {
            ...
            {
                Results = Results.OrderByDescending(s => s.OrderId);
                Results = Results.Skip(pageIndex * pageSize).Take(pageSize);
            }
            else
            {
                Results = Results.OrderBy(s => s.OrderId);
                Results = Results.Skip(pageIndex * pageSize).Take(pageSize);
            }
            var jsonData = new
            {
        ...
        [HttpPost]
        public string Create([Bind(Exclude = "OrderId")] Order obj)
        {
            string msg;
            try
            {
